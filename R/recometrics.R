...
calc.reco.metrics <- function(
    ...
    if (cumulative) {
        if (!inherits(A, "float32")) {
            ...
            if (precision) res$p_at_k@Data <- t(matrix(res$p_at_k@Data, nrow=k))
            if (trunc_precision) res$tp_at_k@Data <- t(matrix(res$tp_at_k@Data, nrow=k))
            if (recall) res$r_at_k@Data <- t(matrix(res$r_at_k@Data, nrow=k))
            if (average_precision) res$ap_at_k@Data <- t(matrix(res$ap_at_k@Data, nrow=k))
            if (trunc_average_precision) res$tap_at_k@Data <- t(matrix(res$tap_at_k@Data, nrow=k))
            if (ndcg) res$ndcg_at_k@Data <- t(matrix(res$ndcg_at_k@Data, nrow=k))
            if (hit) res$hit_at_k@Data <- t(matrix(res$hit_at_k@Data, nrow=k))
            if (rr) res$rr_at_k@Data <- t(matrix(res$rr_at_k@Data, nrow=k))
